...
class GraphQLDocument {
  constructor(source) {
    ...
    try {
      ...
      const range = source_1.rangeInContainingDocument(
        source,
        getDiagnostics_1.getRange(error.locations[0], source.body)
      );
  ...
  containsPosition(position) {
    ...
    const end = source_1.positionFromSourceLocation(
      this.source,
      location_1.getLocation(this.source, this.source.body.length)
    );
...
function extractGraphQLDocuments(document, tagName = "gql") {
  switch (document.languageId) {
    case "graphql":
      return [
        new GraphQLDocument(
          new graphql_1.Source(document.getText(), document.uri)
        )
      ];
    case "javascript":
