...
exports.defaultValidationRules = [
  ...
  ...graphql_1.specifiedRules.filter(
    rule => !specifiedRulesToBeRemoved.includes(rule)
  )
];
function getValidationErrors(
  ...
  document,
  fragments,
  rules = exports.defaultValidationRules
) {
  ...
  graphql_1.visit(
    document,
    graphql_1.visitWithTypeInfo(typeInfo, graphql_1.visitInParallel(visitors))
  );
...
function NoMissingClientDirectives(context) {
  ...
  const executionContext = execute_1.buildExecutionContext(
    schema,
    root,
    Object.create(null),
    Object.create(null),
  ...
  function visitor(node) {
    ...
    switch (node.kind) {
      ...
      case graphql_1.Kind.FRAGMENT_DEFINITION:
        ...
        const fields = graphql_2.simpleCollectFields(
          executionContext,
          node.selectionSet,
          Object.create(null),
          Object.create(null)
        ...
        const codeAction = {
          ...
          edits: [
            vscode_languageserver_1.TextEdit.insert(
              source_1.positionFromSourceLocation(
                source,
                graphql_1.getLocation(source, locToInsertDirective)
              ),
              " @client"
            )
          ]
