...
class EndpointSchemaProvider {
  ...
  async resolveSchema() {
    ...
    const { data, errors } = await apollo_link_1
      .toPromise(
        apollo_link_1.execute(apollo_link_http_1.createHttpLink(options), {
      ...
      .catch(e => {
        ...
        if (
          url === config_1.DefaultServiceConfig.endpoint.url &&
  ...
  async resolveFederatedServiceSDL() {
    ...
    `;
    const { data, errors } = await apollo_link_1.toPromise(
      apollo_link_1.execute(apollo_link_http_1.createHttpLink(options), {
