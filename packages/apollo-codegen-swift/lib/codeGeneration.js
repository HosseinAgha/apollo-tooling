    ...
    this.classDeclaration(
      ...
      () => {
        ...
        if (this.context.options.generateOperationIds) {
          const { operationId } = generateOperationId_1.generateOperationId(
            operation,
            fragments,
            fragmentsReferenced
          );
        ...
        if (fragmentsReferenced.size > 0) {
          ...
          fragmentsReferenced.forEach(fragmentName => {
            this.print(
              language_1.swift`.appending(${this.helpers.structNameForFragmentName(
                fragmentName
              )}.fragmentDefinition)`
            );
          });
  ...
  structDeclarationForVariant(
    ...
    this.structDeclaration(
      ...
      () => {
        ...
        this.printNewlineIfNeeded();
        this.printOnNewline(
          language_1.swift`public static let selections: [GraphQLSelection] = `
        );
        if (typeCase) {
