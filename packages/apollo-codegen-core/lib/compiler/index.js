...
class Compiler {
  ...
  compileOperation(operationDefinition) {
    ...
    const rootType = graphql_2.getOperationRootType(
      this.schema,
      operationDefinition
    );
    return {
  ...
  compileFragment(fragmentDefinition) {
    ...
    const type = graphql_1.typeFromAST(
      this.schema,
  ...
  compileSelectionSet(
    selectionSetNode,
    parentType,
    possibleTypes = this.possibleTypesForType(parentType),
    ...
    switch (selectionNode.kind) {
      case graphql_1.Kind.FIELD: {
        ...
        const alias = selectionNode.alias
          ...
          : undefined;
        const fieldDef = graphql_2.getFieldDef(
          this.schema,
          parentType,
          selectionNode
        );
        ...
        let field = {
          ...
          type: fieldType,
          description:
            !graphql_2.isMetaFieldName(name) && description
              ? description
              : undefined,
          isDeprecated,
