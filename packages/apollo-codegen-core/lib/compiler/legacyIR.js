...
class LegacyIRTransformer {
  ...
  transformIR() {
    ...
    for (const [fragmentName, fragment] of Object.entries(
      ...
      const { selectionSet, type } = fragment,
        fragmentWithoutSelectionSet = __rest(fragment, [
          "selectionSet",
          "type"
        ]);
      fragments[fragmentName] = Object.assign(
        Object.assign(
          { typeCondition: type, possibleTypes: selectionSet.possibleTypes },
  ...
  transformSelectionSetToLegacyIR(selectionSet) {
    ...
    const fields = this.transformFieldsToLegacyIR(
      collectAndMergeFields_1.collectAndMergeFields(typeCase.default, false)
    );
    const inlineFragments = typeCase.variants.flatMap(variant => {
      const fields = this.transformFieldsToLegacyIR(
        collectAndMergeFields_1.collectAndMergeFields(variant, false)
      );
      if (
        ...
        return undefined;
      const fragmentSpreads = this.collectFragmentSpreads(
        selectionSet,
        variant.possibleTypes
      ).map(fragmentSpread => fragmentSpread.fragmentName);
    ...
    const fragmentSpreads = this.collectFragmentSpreads(selectionSet).map(
